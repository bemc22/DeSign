...
def main():
    ...
    print("THRESHOLD OPTIONS LOAD", threshold_opts)
    #* create model
    if len(args.gpus)==1:
        model = eval(model_zoo+args.model)(num_classes=num_classes, threshold_opts=threshold_opts).cuda()
    else: 
        model = nn.DataParallel(eval(model_zoo+args.model)(num_classes=num_classes, threshold_opts=threshold_opts))
    if not args.resume:
